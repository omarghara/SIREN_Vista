...
# CIFAR-10 Spatial-Functa inner-5 pipeline for an arbitrary completed SIREN
# checkpoint. Mirrors the vanilla/softlip inner-5 scripts: make a 5-step
# functaset, train the best-sweep CNN classifier, then run CIFAR-10 spatial PGD.

set -euo pipefail

MODEL_LABEL="${MODEL_LABEL:?set MODEL_LABEL, e.g. warm_readout_cap90_lam1}"
SIREN_CKPT="${SIREN_CKPT:?set SIREN_CKPT to model_cifar10/.../modSiren.pth}"

FUNCTASET_STEM="${FUNCTASET_STEM:-${MODEL_LABEL}_inner5}"
RUN_ROOT="${RUN_ROOT:-runs/cifar10_spatial_inner5_warmstart_models/${MODEL_LABEL}}"
...
MAKESET_VARIANT="${MAKESET_VARIANT:-vanilla}"
...
echo "== CIFAR-10 ${MODEL_LABEL} inner-${MAKE_ITERS} pipeline =="
echo "  run root      : ${RUN_ROOT}"
echo "  checkpoint    : ${SIREN_CKPT}"
echo "  data path     : ${DATA_PATH}"
echo "  make fit      : steps=${MAKE_ITERS}, lr=${MAKE_LR}, optim=${MAKE_INNER_OPTIM}, variant=${MAKESET_VARIANT}"
...
if [[ "${RUN_MAKESET}" == "1" ]]; then
    ...
    if [[ -f "${TRAIN_ALL}" && -f "${TEST_PKL}" ]]; then
        ...
        python -u makeset.py \
            ...
            --save-train-all \
            --device "${MAKE_DEVICE}" \
            --variant "${MAKESET_VARIANT}" 2>&1 | tee "${MAKE_LOG}"
    fi
fi
...
# CIFAR-10 ${MODEL_LABEL} Spatial-Functa Inner-5 Run Config
...
- makeset variant: \`${MAKESET_VARIANT}\`
- classifier type: \`cnn\`
