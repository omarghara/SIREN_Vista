...
# CIFAR-10 Spatial-Functa vanilla inner-5 pipeline.
#
...
MODEL_LABEL="vanilla_e512"
FUNCTASET_STEM="${FUNCTASET_STEM:-vanilla_e512_inner5}"
RUN_ROOT="${RUN_ROOT:-runs/cifar10_spatial_inner5_make5_clfbest_v1/${MODEL_LABEL}}"

MODEL_SLUG="functa_like_cifar10_spatial_paper_siren_h256_md1024_d6_lat8x16_freq10.0_nearest_lc_norm01_extlr3e-05_e512_inner3_moptsgd_adamphi3_lr1e-02_train50000_test10000"
...
echo "== CIFAR-10 vanilla inner-${MAKE_ITERS} pipeline =="
...
if [[ "${RUN_MAKESET}" == "1" ]]; then
    ...
    if [[ -f "${TRAIN_ALL}" && -f "${TEST_PKL}" ]]; then
        ...
        python -u makeset.py \
            ...
            --save-train-all \
            --device "${MAKE_DEVICE}" \
            --variant vanilla 2>&1 | tee "${MAKE_LOG}"
    fi
fi
